# Calculation of heat index using weathermetrics, but removing their rounding and changing a threshold from hi>=79 to hi>=80

heat.index.algorithm2 <- function (t = NA, rh = NA) 
{
  if (is.na(rh) | is.na(t)) {
    hi <- NA
  }
    ...
    hi <- 0.5 * (alpha + t)
    if (hi >= 80) { # changed to be 80 instead of 79
...
heat.index2 <- function (t = NA, dp = c(), rh = c(), temperature.metric = "fahrenheit", output.metric = NULL, round = 2) {
  require(weathermetrics)
    ...
    hi <- mapply(heat.index.algorithm2, t = t, rh = rh) # replaced heat.index.algorithm with heat.index.algorithm2
    if (output.metric == "celsius") {
      hi <- fahrenheit.to.celsius(hi, round = round)
    }
